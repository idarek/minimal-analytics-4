(function() {
    ...
    function sPr() {
        ...
        return scrollable > 0 ? (docEl.scrollTop||docBody.scrollTop) / scrollable * 100 : 0;
    }
    ...
    doc.addEventListener("click", function(e) {
        ...
        if (el && el.getAttribute("href")) {
            ...
            const ext = lastDotIndex !== -1 ? file.substring(lastDotIndex + 1).toLowerCase() : "";
            const cleanFilename = lastDotIndex !== -1 ? file.substring(0, lastDotIndex) : file;

            // Computed once for both branches
            const linkText = el.textContent ? el.textContent.trim().substring(0, 100) : "";

            if (el.hasAttribute("download") || config.ext.includes(ext)) {
                enFdl = true;
                a(ext || undefined, cleanFilename || undefined, linkText, url.replace(docLoc.origin, ""));
                enFdl = false;
            } 
